...
void dump_frame(void)
{
    ...
    if( ! renderedTexture )
    {
        if( ! dump_buffer )
        {
            ...
            dump_buffer = (int*)calloc(resolution.x * resolution.y * sizeof(int), sizeof(resolution.x * resolution.y * sizeof(int)));
#else
...
void show_dumped_frame(void)
{
    ...
    {
        vec4 r = (vec4) { -.5,.5,  .0,.0 };
        on_GLES2_Render_icon(USE_COLOR, renderedTexture, 2, r);
    }
}
